      ...
      <div className="p-6">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-base font-semibold text-gray-800">Resumo por equipe</h3>
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  ...
                  <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Líder</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Área</th>
                  <th className="text-center px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Etapa</th>
                  <th className="text-center px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Tarefas</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Status</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Inscrito em</th>
                </tr>
              </thead>
              <tbody>
                {mockTeams.map((team) => {
                  ...
                  return (
                    <tr key={team.id} className="border-b border-gray-100">
                      <td className="px-4 py-3 text-sm font-medium text-gray-800">{team.ideaName}</td>
                      <td className="px-4 py-3 text-sm text-gray-600">{team.leader.name}</td>
                      <td className="px-4 py-3 text-sm text-gray-600">{team.area}</td>
                      <td className="px-4 py-3 text-sm text-gray-600 text-center">{team.currentStage}/6</td>
                      <td className="px-4 py-3 text-sm text-gray-600 text-center">{approved}/{teamTasks.length}</td>
                      <td className="px-4 py-3">
                        <span className={`text-xs font-medium ${team.status === "pronta_inovamf" ? "text-green-600" : team.status === "ativa" ? "text-blue-600" : "text-gray-500"}`}>
                          {team.status === "pronta_inovamf" ? "Pronta" : team.status === "ativa" ? "Ativa" : team.status}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-sm text-gray-500">{team.createdAt}</td>
                    </tr>
                  );
                })}
              </tbody>
...
function StatCard({ icon, label, value }: { icon: React.ReactNode; label: string; value: number | string }) {
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-4">
      <div className="flex items-center gap-3 mb-2">{icon}<span className="text-xs text-gray-500">{label}</span></div>
