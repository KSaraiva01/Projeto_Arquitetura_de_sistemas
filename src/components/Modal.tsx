...
export const Modal: React.FC<ModalProps> = ({ open, onClose, title, subtitle, children, footer, size = 'md' }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-ink-950/50 p-0 backdrop-blur-sm sm:items-center sm:p-4">
      <div
        className={`relative flex max-h-[90vh] w-full ${width} flex-col rounded-t-2xl bg-white shadow-2xl sm:rounded-2xl animate-[fadeIn_.15s_ease-out]`}
        role="dialog"
        aria-modal="true"
      >
        <div className="flex items-start justify-between border-b border-slate-100 px-5 py-4">
          ...
          <button
            onClick={onClose}
            className="rounded-lg p-1.5 text-ink-700/50 transition hover:bg-slate-100 hover:text-ink-900"
            aria-label="Fechar"
          >
            <X size={18} />
          </button>
