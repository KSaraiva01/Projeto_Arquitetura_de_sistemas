  ...
  return (
    <Layout title="Minhas tarefas">
      <div className="mb-5 flex items-center justify-between gap-3">
        <p className="text-sm text-ink-700/60">Tarefas da equipe {team.ideaName}</p>
      ...
      <div className="space-y-3">
        {tasks.length === 0 && <Card className="text-center text-sm text-ink-700/50">Nenhuma tarefa encontrada para este filtro.</Card>}
        {tasks.map((t) => (
          <Card key={t.id}>
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div className="max-w-xl">
                <p className="font-semibold text-ink-900">{t.title}</p>
                <p className="mt-1 text-sm text-ink-700/60">{t.description}</p>
                <div className="mt-2 flex flex-wrap items-center gap-3 text-xs text-ink-700/50">
                  <span className="flex items-center gap-1"><Clock size={12} /> Prazo: {new Date(t.dueDate + 'T00:00:00').toLocaleDateString('pt-BR')}</span>
                  <span>Etapa: {stageById(t.stageId)?.shortTitle}</span>
                </div>
              </div>
